...
def constant_sampling(time_start, time_end, input_signal, input_freq, if_draw=False):

    fs = user_communication.get_quantization_frequency()
    fs = int(input_freq / fs)
    signal_sampled = input_signal[::fs]
    if if_draw:
        samples_num = len(signal_sampled)
        main.draw_graph("Constant sampling", time_start, time_end, input_freq * (time_end - time_start) / fs, samples_num, signal_sampled)

    return time_start, time_end, signal_sampled, fs


def constant_quantization_with_clip(time_start, time_end, input_signal, input_freq, if_draw=True):
    clip_value = user_communication.get_clip_value()
    new_signal = copy.deepcopy(input_signal)
    for x in range(len(input_signal)):
        original_value = input_signal[x]
        v = original_value % clip_value
        v1 = original_value - v
        new_signal[x] = v1

    if if_draw:
        t = np.linspace(int(time_start), int(time_end), int(input_freq * (time_end - time_start)))
        fig, ax = plt.subplots()
    ...
    return time_start, time_end, new_signal, input_freq, input_signal
...
def zero_order_hold_reconstruction(time_start, time_end, input_signal, input_freq, original_signal, if_draw=False):
    frequency = 5000
    new_signal = np.zeros(frequency * int(time_end - time_start))
    original_samples_num = len(input_signal)
    ...
    for x in range(original_samples_num):
        for y in range(ratio):
            new_signal[x * ratio + y] = copy.deepcopy(input_signal[x])

    if if_draw:
        t1 = np.linspace(int(time_start), int(time_end), int(input_freq * (time_end - time_start)))
        t2 = np.linspace(int(time_start), int(time_end), int(frequency * (time_end - time_start)))
        fig, ax = plt.subplots()
        ax.plot(t1, input_signal, label="input")
...
original_signal, t1, t2 = main.sinus_signal(original_freq, time_start, time_end, True)

t1, t2, signal_sampled, sampled_freq = constant_sampling(time_start, time_end, original_signal, original_freq, True)

reconstructed_signal_ZOH = zero_order_hold_reconstruction(time_start, time_end, signal_sampled, sampled_freq, original_signal, True)
# signal, time_start, time_to_end = main.sinus_signal(1000, 0, 2, False)
